...
var minidata = [
  {
    ...
    phone: "(956) 428-2996",
    color:
      "<button class='btn' style='color:rgb(201,142,97);'>Blue Bottle cardigan</button>"
  },
  {
    first: "Silva",
    last: "Alexander",
    ...
    phone: "(823) 415-2224",
    color:
      "<button class='btn' style='color:rgb(113,105,251);'>Freegan tilde</button>"
  },
  {
    first: "Hopkins",
    last: "Wong",
    ...
    phone: "(814) 488-2063",
    color:
      "<button class='btn' style='color:rgb(133,126,66);'>bespoke gastropub</button>"
  },
  {
    first: "Harriet",
    last: "Sherman",
    ...
    phone: "(887) 497-3612",
    color:
      "<button class='btn' style='color:rgb(195,89,110);'>dreamcatcher fingerstache</button>"
  },
  {
    first: "Stacie",
    last: "Bartlett",
    ...
    phone: "(957) 412-3261",
    color:
      "<button class='btn' style='color:rgb(164,84,221);'>Etsy beard</button>"
  },
  {
    first: "Lindsay",
    last: "Meyer",
    ...
    phone: "(956) 428-2996",
    color:
      "<button class='btn' style='color:rgb(201,142,97);'>Austin meggings</button>"
  },
  {
    first: "Shelley",
    last: "Alexander",
    ...
    phone: "(823) 415-2224",
    color:
      "<button class='btn' style='color:rgb(113,105,251);'>Kitsch banjo</button>"
  },
  {
    first: "Rita",
    last: "Wong",
    ...
    phone: "(814) 488-2063",
    color:
      "<button class='btn' style='color:rgb(133,126,66);'>Brooklyn authentic</button>"
  },
  {
    first: "Joe",
    last: "Sherman",
    ...
    phone: "(887) 497-3612",
    color:
      "<button class='btn' style='color:rgb(195,89,110);'>Pickled cray scenester</button>"
  },
  {
    first: "Jane",
    last: "Bartlett",
    ...
    phone: "(957) 412-3261",
    color:
      "<button class='btn' style='color:rgb(164,84,221);'>8-bit migas</button>"
  }
];
  ...
  suite("Remote Data Paradigm", function() {
    ...
    suite("Page 1 - remoteData1", function() {
      ...
      test("Default pagination size is 10", function() {
        assert.equal(
          currentTableFixture.pageSize,
          10,
          "Default page size should be 10 rows."
        );
      });
      ...
      test("Pagination shows 1-10 of 100", function() {
        var paginationSpan = currentTableFixture.querySelector(
          ".summary.px-pagination"
        );
        var paginationTextString = paginationSpan.textContent
          .replace(/\s\s*/g, " ")
          .trim();
        assert.equal(
          paginationTextString,
          "1-10 of 100",
          "Shows correct pagination counts."
        );
      });

      test("Page 1 should be selected", function() {
        var paginationPager = currentTableFixture.querySelector(
          ".px-pagination"
        );
        // note: classList does not have prototype Array methods
        var page1IconClassList = paginationPager.children[0].classList
          .toString()
          .split(" ");
        assert.equal(
          currentTableFixture.querySelector(
            "div.px-pagination > span.btn.btn--tertiary.u-ml.style-scope.px-pagination"
          ).textContent,
          "1",
      ...
      test("Page 2 should NOT be selected", function() {
        ...
        // note: classList does not have prototype Array methods
        var page2IconClassList = paginationPager.children[1].classList
          .toString()
          .split(" ");
        assert.equal(
          currentTableFixture.querySelectorAll(
            "div.px-pagination > span.btn.btn--tertiary.u-ml.style-scope.px-pagination"
          ).length,
          1,
      ...
      test("Clicking Next Page button fires a `px-page-change-intent` event", function(done) {
        ...
        var pageChangeButton = dataTable.querySelector(
          ".px-pagination .btn#next"
        );
      ...
      test("Updating properties triggers appearance of Page 2", function() {
        ...
        var paginationSpan = dataTable.querySelector(
          ".px-pagination.summary strong"
        );
        var paginationTextString = paginationSpan.textContent
          .replace(/\s\s*/g, " ")
          .trim();
        ...
        // note: classList does not have prototype Array methods
        var pageNIconClass = paginationPager.children[page - 1].classList
          .toString()
          .split(" ");
        assert.equal(
          currentTableFixture.querySelector(
    ...
    suite("Rows per Page Dropdown - remoteData3", function() {
      ...
      test("On change, it fires a `px-page-size-change-intent` event", function(done) {
        var dataTable = currentTableFixture;
        var paginationElement = dataTable.querySelector(
          ".pagination px-pagination#pagination"
        );
        // var pageSizeSelectDropdown = dataTable.querySelector('.pagination px-pagination#pagination px-dropdown');

        let eventCounter = 0;

        expectedPageSizes = [10, 20, 20];

        dataTable.addEventListener("px-page-size-change-intent", evt => {
          assert(true, "Event is triggered");
          var val = _typeofComparison(evt.detail, "string")
            ? parseInt(evt.detail, expectedPageSizes[eventCounter])
            : evt.detail;
          assert.equal(
            val,
            expectedPageSizes[eventCounter],
            "Requesting page size of 10"
          );
          eventCounter++;
          if (eventCounter <= 1) done();
        });
        ...
        paginationElement.dispatchEvent(new Event("selected-changed"), {
          target: { value: 10 }
        });
      });
      ...
      test("Updates value accordingly when value is passed in", function() {
        var dataTable = currentTableFixture;
        var pageSizeSelectDropdown = dataTable.querySelector(
          ".pagination px-pagination#pagination px-dropdown"
        );
    ...
    suite("Filtering - remoteDataFiltering1", function() {
      ...
      test("Send event with combined filter string for one column", function(done) {
        var dataTable = currentTableFixture;
        var lastNameFilterSelector =
          "div > div.tr.tr--filter > :nth-child(2) > input";
        ...
        dataTable.addEventListener("px-filter-change-intent", evt => {
          ...
          assert.equal(
            evt.detail,
            '[{"name":"last","userEntry":"ab"}]',
            "Requesting filter change"
          );
          done();
        });
      ...
      test("Send event with combined filter string for all columns", function(done) {
        var filters = [
          { name: "first", userEntry: "ab" },
          { name: "last", userEntry: "cd" }
        ];

        var dataTable = currentTableFixture2;
        var ahaTable = dataTable.querySelector("#dataTable");

        var firstNameFilterSelector =
          "div > div.tr.tr--filter > :nth-child(1) > input";
        ...
        var imageFilterSelector =
          "div > div.tr.tr--filter > :nth-child(3) > input";
        ...
        dataTable.addEventListener("px-filter-change-intent", evt => {
          ...
          assert.equal(
            ...
            "Requesting filter change on multiple columns"
          );
          done();
        });
    ...
    suite("Sorting - remoteDataSorting1", function() {
      ...
      test("Send event with combined sort string for one column", function(done) {
        var dataTable = currentTableFixture;
        var sortHeadingSelector =
          "div > div.tr .th:nth-of-type(2) .column-head.sorted-text-hover";
        ...
        dataTable.addEventListener("px-sort-change-intent", evt => {
          ...
          assert.equal(
            evt.detail,
            '[{"name":"last","direction":"ascending"}]',
            "Requesting sort change"
          );
          done();
        });
      ...
      test("Send event with combined sort string for one column", function(done) {
        ...
        dataTable.addEventListener("px-sort-change-intent", evt => {
          ...
          assert.equal(
            evt.detail,
            '[{"name":"last","direction":"descending"}]',
            "Requesting sort change"
          );
          done();
        });
